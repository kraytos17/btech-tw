...
prior to this work. Beaulieu et al.~\cite{beaulieu2015simon} reported
SPECK-64/128 performance on 8-bit AVR microcontrollers at 164 cycles per
byte, demonstrating the cipher's suitability for low-end platforms. On
32-bit ARM Cortex-M3 processors, SPECK-128/128 achieved approximately
16 CPB, benefiting from the wider datapath.

The NISTIR 8114 report~\cite{mckay2017nist} surveyed a broad range of
lightweight candidates across multiple platforms, providing a comprehensive
comparison of throughput, memory, and security properties. That survey
identified PRESENT as the most hardware-efficient option but noted its
poor software performance, and highlighted the need for more benchmarking
on contemporary microcontrollers such as the ARM Cortex-M0+ family.

Other notable studies include the ISO/IEC standardization of PRESENT
(ISO/IEC 29192-2:2019)~\cite{iso29192} and the ongoing cryptanalysis
of the SPECK family~\cite{ashur2018simon}, which have established
confidence in the security margins of these algorithms. However, no
prior study has provided a side-by-side comparison of PRESENT, SPECK,
and ASCON on the RP2040 platform using Rust implementations, which
...
\begin{table}[H]
    ...
    \begin{tabular}{lcccc}
        ...
        SPECK-64/96 & 64 & 96 & 26 & 7, 2 \\
        SPECK-64/128 & 64 & 128 & 27 & 7, 2 \\
...
\end{enumerate}
The 64-bit rate and 256-bit capacity split provides 128-bit security while
...
\texttt{thumbv6m-none-eabi} target triple (ARM Cortex-M0+). The
implementations follow a common interface: a \texttt{BlockCipher} trait
with \texttt{encrypt\_block()} and \texttt{decrypt\_block()} methods,
enabling drop-in benchmarking across algorithms.
...
in a fixed-size array on the stack.
...
64-bit word. The two permutation variants ($p^a$ with 12 rounds and
$p^b$ with 6 rounds) share the same round function, differing only
...
Additionally, SPECK's NSA origin has led to scrutiny and its rejection
from ISO/IEC standardization, which may be a concern for regulated
deployments. Nevertheless, SPECK remains one of the most efficient
