...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
            >=Stealth,
            node distance=1cm,
            box/.style={draw, minimum width=7cm, minimum height=1.0cm, align=center},
            smallbox/.style={draw, minimum width=3.2cm, minimum height=0.8cm, align=center}
        ]

        \node[box] (state) {ASCON State (320 bits)};
        \draw (state.south west) -- ++(3.5,0) node[pos=0.5, below] {Rate};
        \draw (state.south west) ++(3.5,0) -- (state.south east) node[pos=0.5, below] {Capacity};

        \node[smallbox, above=1.2cm of state.west, anchor=west] (aad) {Associated Data};
        \draw[->] (aad.south) |- (state.west);

        \node[smallbox, below=1.4cm of state.west, anchor=west] (pt) {Plaintext Blocks};
        \node[smallbox, right=4.0cm of pt] (ct) {Ciphertext Blocks};
        \draw[->] (pt.north) |- (state.west);
        \draw[->] (state.east) |- (ct.north);

        \node[smallbox, right=1.3cm of state] (perm) {Permutation $p$};
        \draw[->] (state.east) -- (perm.west);
        \draw[->] (perm.west) -- (state.east);

        \node[smallbox, below=0.8cm of ct] (tag) {Authentication Tag};
        \draw[->] (state.south) |- (tag.west);

    \end{tikzpicture}
    \caption{High-level view of ASCON authenticated-encryption using a sponge-duplex construction.}
    \label{fig:ascon-sponge}
\end{figure}
