...
\begin{frame}{Completed Tasks and Pending Tasks}
\small
...
\begin{frame}{Host Machine Benchmarks}
\small
Benchmarks on x86\_64 validate the implementations; absolute values differ from Pico, but relative comparisons remain useful.

\vspace{0.2cm}
\begin{tabular}{lrrr}
\toprule
Algorithm & ns/block & CPB & Mbps \\
\midrule
...
\begin{frame}{Raspberry Pi Pico Benchmarks}
\small
Primary benchmarks were conducted on Raspberry Pi Pico (ARM Cortex-M0+, 133 MHz).
...
This study investigates lightweight cryptographic algorithms for resource-constrained embedded platforms.

\vspace{0.3cm}
PRESENT, SPECK, and ASCON were evaluated using Rust-based implementations on Raspberry Pi Pico.

\vspace{0.3cm}
Results show that SPECK provides superior performance, PRESENT offers minimal memory footprint, and ASCON delivers strong modern security with functional completeness.
\end{frame}
